...
//       throw new Error(
//         `GameMaster wallet address mismatch: ${this.wallet.address} != ${this.gmId}`
//       );
//     }
...
//       pvpvaiWsServerUrl: this.endpoint,
//       roomId: this.roomId,
//       auth: {
//         walletAddress: this.wallet.address,
//         agentId: this.gmNumericId,
...
//     try {
//       const response = await axios.get<RoundResponse>(
//         `${this.endpoint}/rooms/${this.roomId}/rounds/active`
//       );
...
//         participants:
//           participantsResp.data.data?.filter(
//             (p: any) =>
...
//         const agents =
//           roundState?.participants?.filter(
//             (p: any) => p.id !== this.gmNumericId
//           ) || [];

//         if (agents.length > 0) {
//           console.log(
//             `Found ${agents.length} agents in room, starting debate...`
//           );
//           this.stopPolling();
...
//     const roundState = await this.getRoundState();
//     const registeredAgents = roundState.participants.map(
//       (p: any) => p.agent_id
//     );
...
//         "Beginning discussion phase. Agents may now engage in debate.",
//         []
//       );
...
//       const topic =
//         "Let's discuss the future of cryptocurrency. What are your thoughts on Bitcoin versus Ethereum?";
...
//           this.wallet
//             .signMessage(JSON.stringify(heartbeatContent))
//             .then((signature) => {
//               this.wsClient?.send({
